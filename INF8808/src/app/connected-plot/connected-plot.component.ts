...
export class ConnectedPlotComponent implements OnInit {
  ...
  private hommeColor = "#ef8606";
  private femmeColor = "#41a1ef";
...
// Set axes (x and y)
  private setAxes(data:any): void{
    ...
    let yAxisGroup = this.svg.append("g").attr("class", "y-axis-group-connected-plot");
    yAxisGroup.append("g")
    .attr("class", "y-axis")
    ...
    let self = this;
    d3.selectAll('.y-axis-group-connected-plot .y-axis .tick')
    .each(function(d, i) {
      d3.select(this).attr('class', 'tick-y-secteur-' + self.trimString(`${d}`));
    });
...
// Create grid (x and y)
  private createGrid(data:any): void {
    let axisLinesVertical = this.svg.append("g")
    .attr("class", "grid-lines-vertical-connected-plot");
    axisLinesVertical.selectAll("path")
      .data(this.x.ticks())
      .enter().append("path")
    ...
    let axisLinesHorizontal = this.svg.append("g")
      .attr("class", "grid-lines-horizontal-connected-plot");
      axisLinesHorizontal.selectAll("path")
      .data(yPosData)
      .enter().append("path")
...
// Create lollipops
  ...
  private addText(x:any, y:any, data:any): void{
    this.svg.append("text")
      ...
      let self = this;
      d3.selectAll("circle[class*='lollipop-homme']")
      .each(function(d:any) {
        self.svg.append("text")
        .text("Homme")
        .attr("x", x(d.homme) < x(d.femme) ? x(d.homme)-60 : x(d.homme)+15)
        .attr("y", y(d.name)+10)
        .attr('class', 'sector-text-connected-plot-homme-'+self.trimString(`${d.name}`))
      ...
      .each(function(d:any) {
        ...
        .attr("x", x(d.homme) < x(d.femme) ? x(d.homme)-60 : x(d.homme)+15)
        .attr("y", y(d.name)+25)
        .attr('class', 'sector-text-connected-plot-homme-number'+self.trimString(`${d.name}`))
        .attr("fill", self.hommeColor)
        .attr("opacity",0)
      });

      d3.selectAll("circle[class*='lollipop-femme']")
      .each(function(d:any) {
        self.svg.append("text")
        .text("Femme")
        .attr("x", x(d.homme) > x(d.femme) ? x(d.femme)-60 : x(d.femme)+15)
        .attr("y", y(d.name)+10)
        .attr('class', 'sector-text-connected-plot-femme-'+self.trimString(`${d.name}`))
      ...
      .each(function(d:any) {
        ...
        .attr("x", x(d.homme) > x(d.femme) ? x(d.femme)-60 : x(d.femme)+15)
        .attr("y", y(d.name)+25)
        .attr('class', 'sector-text-connected-plot-femme-number'+self.trimString(`${d.name}`))
        .attr("fill", self.femmeColor)
        .attr("opacity",0)
      });
  }
...
private handleHover(d:any): void {
  ...
  d3.selectAll("circle[class*='lollipop-femme']").style('opacity', reducedOpacity);
  d3.selectAll("circle[class*='lollipop-homme']").style('opacity', reducedOpacity);
  d3.selectAll("path[class*='lollipop-line']").style('opacity', reducedOpacity);
  ...
  d3.selectAll(`[class*='${this.trimString(`${d.target.__data__.name}`)}']`).style('opacity', 1)
  }
