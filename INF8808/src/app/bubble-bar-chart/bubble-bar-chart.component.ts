  ...
  ngOnInit(): void {
    d3.text(this.dataPaths[this.currentOptionIndex]).then((data) => {
      ...
      this.button.on('click', () => {
        ...
        this.currentForce =
          this.currentForce === GENRE_Y_FORCE ? NATURE_Y_FORCE : GENRE_Y_FORCE;
        this.currentTextCutoff =
          this.currentTextCutoff === GENRE_TEXT_CUTOFF
            ? NATURE_TEXT_CUTOFF
            : GENRE_TEXT_CUTOFF;

        d3.selectAll(
          'figure#bar svg g :not(.button, .button-text, .button-text-first-row, .button-text-second-row'
        ).remove();
        d3.text(this.dataPaths[this.currentOptionIndex]).then((data) => {
          this.data = [];
          let rowIndex = 0;
          ...
          const tspan = this.svg
            .select('.button')
            .select('.button-text')
            .selectAll('tspan');

          tspan
            .filter('.button-text-first-row')
            .text(
              'VOIR ' +
                this.options[this.currentOptionIndex]
                  .split(' ')
                  .slice(0, this.currentTextCutoff)
                  .join(' ')
            );

          tspan
            .filter('.button-text-second-row')
            .text(
              this.options[this.currentOptionIndex]
                .split(' ')
                .slice(this.currentTextCutoff)
                .join(' ')
            );
        });
      });
  ...
  private drawButton(): any {
    ...
    button
      ...
      .on('mouseenter', function (event: any) {
        d3.select(event.target).attr('stroke', '#674FFABB');
      })
